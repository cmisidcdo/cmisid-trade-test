            ...
            <div class="card-body p-4">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-bordered text-center global-table">
                        ...
                        <tbody>
                            @forelse($assignedpracticals as $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-sm btn-outline-primary me-1" wire:click='readAssignedPractical({{$item->id}})' title="Edit">
                    ...
                    <div>
                        ...
                        <div class="modal fade" id="assignedPracticalModal" tabindex="-1" aria-labelledby="assignedPracticalModalLabel" aria-hidden="true"  wire:ignore.self>
                            <div class="modal-dialog modal-dialog-centered modal-lg">
                                <div class="modal-content" style="border-radius: 12px;">
                                    <div class="modal-body p-4">
                                        <h5 class="text-center mb-4 fw-bold text-dark">{{$editMode ? 'Update Practical Exam Schedule' : 'Add Schedule for Practical Test'}}</h5>
                                        <form wire:submit.prevent="{{$editMode ? 'updateAssignedPractical' : 'createAssignedPractical'}}">
                                            <div class="row g-3">
                                            ...
                                            <div class="d-flex justify-content-between">
                                                ...
                                                @if($editMode)
                                                <button type="button" class="btn btn-primary btn-sm d-flex align-items-center px-3" wire:click='updatePracticalScenario({{$assignedpracticalId}})'>
                                                    <i class="bi bi-check-circle me-2"></i> Update Scenarios
                                                </button>
                                                @endif

                                                <button type="submit" class="btn btn-primary btn-sm d-flex align-items-center px-3">
                                                    <i class="bi bi-check-circle me-2"></i> {{$editMode ? 'Save Changes' : 'Create and Proceed to Scenarios'}}
                                                </button>
                                            </div>
                                        </form>
                                    </div>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="d-flex justify-content-end gap-2 mt-4">
                                        <button
                                            ...
                                            <i class="bi bi-arrow-left me-1"></i> Back to Practical 
                                        </button>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    $wire.on('hide-assignedPracticalModal', () => {
        $('#assignedPracticalModal').modal('hide');
    });

    $wire.on('show-assignedPracticalModal', () => {
        $('#assignedPracticalModal').modal('show');
    });

    $wire.on('show-candidatesModal', () => {
        bootstrap.Modal.getInstance(document.getElementById('assignedPracticalModal')).hide();
        new bootstrap.Modal(document.getElementById('candidatesModal')).show();
    });

    $wire.on('hide-candidatesModal', () => {
        new bootstrap.Modal(document.getElementById('assignedPracticalModal')).show();
